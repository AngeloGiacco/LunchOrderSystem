<?php
  ...
  $DateRequired = $_GET['DateReq'];
  include_once('connection.php');
  $stmt = $conn->prepare('SELECT * FROM Orders WHERE DateRequired = :DateRequired and StudentID = :StudentID');
  $stmt->bindParam(':StudentID',$StudentID);
  $stmt->bindParam(':DateRequired',$DateRequired);
  $stmt->execute();
  $result = $stmt->fetch(PDO::FETCH_ASSOC);
  print_r($result)
  /*
  $stmt = $conn->prepare('DELETE FROM Orders WHERE DateRequired = :DateRequired and StudentID = :StudentID');
  $stmt->bindParam(':StudentID',$StudentID);
  $stmt->bindParam(':DateRequired',$DateRequired);
  $stmt->execute();*/
?>
...
<body>
    ...
    <div class="border-top">
        ...
        <h3>SUCCESS!</h3>
        <?php echo("<p>Your order has been deleted!</p>");?>
